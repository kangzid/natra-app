...
class HrisNewsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('banner_file')) {
            ...
            // Only update if it's a new base64 (data:image/...)
    ...
    public function destroy(Request $request, $id)
    {
        ...
        // Delete encrypted banner file if exists
        if ($news->banner_path) {
            EncryptedStorageService::deleteEncrypted($news->banner_path);
        }
...
console.log('Successfully updated HrisNewsController with encrypted file cleanup on delete and update!');
