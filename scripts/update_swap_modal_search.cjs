...
const pageContent = `<script lang="ts">
    ...
    let selectedDateCheckedEmpIds = $state<number[]>([]);
    let bulkDateShiftId = $state<number | string>('');
    let isApplyingBulkDate = $state(false);
    ...
    let isAllDateEmployeesChecked = $derived(
        selectedDateEmployees.length > 0 && 
        selectedDateEmployees.every((item: any) => selectedDateCheckedEmpIds.includes(item.employee.id))
    );

    function toggleAllDateEmployees() {
        if (isAllDateEmployeesChecked) {
            const visibleIds = new Set(selectedDateEmployees.map((item: any) => item.employee.id));
            selectedDateCheckedEmpIds = selectedDateCheckedEmpIds.filter(id => !visibleIds.has(id));
        } else {
            const visibleIds = selectedDateEmployees.map((item: any) => item.employee.id);
            selectedDateCheckedEmpIds = Array.from(new Set([...selectedDateCheckedEmpIds, ...visibleIds]));
        }
    }

    function toggleSingleDateEmployee(empId: number) {
        if (selectedDateCheckedEmpIds.includes(empId)) {
            selectedDateCheckedEmpIds = selectedDateCheckedEmpIds.filter(id => id !== empId);
        } else {
            selectedDateCheckedEmpIds = [...selectedDateCheckedEmpIds, empId];
        }
    }

    async function handleApplyBulkDateShift(targetShiftId: string | number) {
        if (selectedDateCheckedEmpIds.length === 0) {
            showToast('Pilih minimal 1 karyawan', 'error');
            return;
        }

        isApplyingBulkDate = true;
        try {
            if (!targetShiftId || targetShiftId === 'remove') {
                const dayAssignments = assignmentsByDate[selectedDate] || [];
                const checkedSet = new Set(selectedDateCheckedEmpIds);
                const toDelete = dayAssignments.filter((a: any) => checkedSet.has(a.employee_id));

                for (const a of toDelete) {
                    await fetch(\`\${PUBLIC_API_URL}/hris/shift-assignments/\${a.id}\`, {
                        method: 'DELETE',
                        headers: { 'Authorization': \`Bearer \${data.token}\`, 'Accept': 'application/json' }
                    });
                }
                showToast(\`Jadwal shift \${toDelete.length} karyawan diatur ke Reguler/Libur\`, 'success');
            } else {
                const res = await fetch(\`\${PUBLIC_API_URL}/hris/shift-assignments\`, {
                    ...
                    body: JSON.stringify({
                        employee_ids: selectedDateCheckedEmpIds,
                        shift_id: Number(targetShiftId),
                        start_date: selectedDate,
                        end_date: selectedDate,
                        notes: 'Penetapan massal tanggal ' + selectedDate
                    })
                });
                const resJson = await res.json();
                if (!res.ok) throw new Error(resJson.message || 'Gagal mengubah shift massal');
                showToast(\`Berhasil mengubah shift \${selectedDateCheckedEmpIds.length} karyawan!\`, 'success');
            }

            selectedDateCheckedEmpIds = [];
            bulkDateShiftId = '';
            await invalidateAll();
        } catch (err: any) {
            showToast(err.message || 'Gagal menerapkan shift massal', 'error');
        } finally {
            isApplyingBulkDate = false;
        }
    }
        ...
        try {
            if (!shiftId || shiftId === 'remove') {
                ...
                if (currentAssign) {
                    ...
                    showToast('Jadwal shift diatur ke Reguler', 'success');
                }
            } else {
                const res = await fetch(\`\${PUBLIC_API_URL}/hris/shift-assignments\`, {
                    method: 'POST',
    ...
    // --- Modal 1: Auto-Generate Shift Roster Cerdas ---
    let isAutoGenerateModalOpen = $state(false);
    let isSubmittingAutoGenerate = $state(false);
    let autoGenForm = $state({
        start_date: todayStr,
        end_date: new Date(todayObj.getFullYear(), todayObj.getMonth() + 1, 0).toISOString().split('T')[0],
        employee_ids: [] as number[],
        shift_ids: [] as number[],
        pattern: 'weekly_rotation',
        off_days: [0],
        rotating_off: false,
        notes: 'Auto-Generated Roster'
    });

    let autoGenSearchQuery = $state('');
    let autoGenDeptFilter = $state('all');

    let autoGenFilteredEmployees = $derived(
        employees.filter((emp: any) => {
            const matchesSearch = !autoGenSearchQuery ||
                (emp.user?.name || '').toLowerCase().includes(autoGenSearchQuery.toLowerCase()) ||
                (emp.department || '').toLowerCase().includes(autoGenSearchQuery.toLowerCase());
            const matchesDept = autoGenDeptFilter === 'all' || emp.department === autoGenDeptFilter;
    ...
    function openAutoGenerateModal() {
        ...
        autoGenForm = {
            ...
            shift_ids: shifts.map((s: any) => s.id),
            pattern: 'weekly_rotation',
            off_days: [0],
            rotating_off: false,
            notes: \`Auto-Generated Roster \${monthNames[currentMonth - 1]} \${currentYear}\`
        };
        isAutoGenerateModalOpen = true;
    }

    function toggleAutoGenEmployee(empId: number) {
        if (autoGenForm.employee_ids.includes(empId)) {
            autoGenForm.employee_ids = autoGenForm.employee_ids.filter(id => id !== empId);
        } else {
            autoGenForm.employee_ids = [...autoGenForm.employee_ids, empId];
        }
    }

    function toggleAutoGenShift(shiftId: number) {
        if (autoGenForm.shift_ids.includes(shiftId)) {
            autoGenForm.shift_ids = autoGenForm.shift_ids.filter(id => id !== shiftId);
        } else {
            autoGenForm.shift_ids = [...autoGenForm.shift_ids, shiftId];
        }
    }

    function toggleOffDay(dayNum: number) {
        if (autoGenForm.off_days.includes(dayNum)) {
            autoGenForm.off_days = autoGenForm.off_days.filter(d => d !== dayNum);
        } else {
            autoGenForm.off_days = [...autoGenForm.off_days, dayNum];
        }
    }

    async function handleExecuteAutoGenerate() {
        if (autoGenForm.employee_ids.length === 0) {
            showToast('Pilih minimal 1 karyawan', 'error');
            return;
        }
        if (autoGenForm.shift_ids.length === 0) {
            showToast('Pilih minimal 1 shift kerja yang digunakan', 'error');
            return;
        }

        isSubmittingAutoGenerate = true;
        try {
            const res = await fetch(\`\${PUBLIC_API_URL}/hris/shift-assignments/auto-generate\`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': \`Bearer \${data.token}\`,
            ...
            const resJson = await res.json();
            if (!res.ok) throw new Error(resJson.message || 'Gagal melakukan auto-generate');

            showToast(resJson.message || 'Auto-generate roster berhasil diterapkan!', 'success');
            isAutoGenerateModalOpen = false;
            await invalidateAll();
        } catch (err: any) {
            showToast(err.message || 'Gagal generate roster', 'error');
        } finally {
            isSubmittingAutoGenerate = false;
        }
    }

    // --- Modal 2: Swap / Tukar Jadwal Antar Karyawan WITH DEDICATED SEARCH ---
    let isSwapModalOpen = $state(false);
    let isSubmittingSwap = $state(false);
    let swapForm = $state({
        employee_id_1: '' as string | number,
        employee_id_2: '' as string | number,
        start_date: todayStr,
        end_date: todayStr
    });

    let searchEmp1 = $state('');
    let searchEmp2 = $state('');

    let filteredSwapEmployees1 = $derived(
        employees.filter((e: any) => {
            const q = searchEmp1.toLowerCase();
            return !q || (e.user?.name || '').toLowerCase().includes(q) || (e.department || '').toLowerCase().includes(q);
        })
    );

    let filteredSwapEmployees2 = $derived(
        employees.filter((e: any) => {
            const q = searchEmp2.toLowerCase();
            return !q || (e.user?.name || '').toLowerCase().includes(q) || (e.department || '').toLowerCase().includes(q);
        })
    );
    ...
    function openSwapModal() {
        ...
        searchEmp2 = '';
        swapForm = {
            employee_id_1: employees.length > 0 ? employees[0].id : '',
            employee_id_2: employees.length > 1 ? employees[1].id : '',
            start_date: selectedDate || todayStr,
            end_date: selectedDate || todayStr
        };
        isSwapModalOpen = true;
    }

    let selectedEmp1Obj = $derived(employees.find((e: any) => e.id === Number(swapForm.employee_id_1)));
    let selectedEmp2Obj = $derived(employees.find((e: any) => e.id === Number(swapForm.employee_id_2)));

    async function handleExecuteSwap() {
        if (!swapForm.employee_id_1 || !swapForm.employee_id_2) {
            showToast('Pilih 2 karyawan yang akan ditukar jadwalnya', 'error');
            return;
        }
        if (Number(swapForm.employee_id_1) === Number(swapForm.employee_id_2)) {
            showToast('Karyawan 1 dan Karyawan 2 harus berbeda', 'error');
            return;
        }

        isSubmittingSwap = true;
        try {
            const res = await fetch(\`\${PUBLIC_API_URL}/hris/shift-assignments/swap\`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': \`Bearer \${data.token}\`,
                ...
                body: JSON.stringify({
                    employee_id_1: Number(swapForm.employee_id_1),
                    employee_id_2: Number(swapForm.employee_id_2),
                    start_date: swapForm.start_date,
                    end_date: swapForm.end_date
                })
            });

            const resJson = await res.json();
            if (!res.ok) throw new Error(resJson.message || 'Gagal menukar jadwal');

            showToast(resJson.message || 'Jadwal berhasil ditukar!', 'success');
            isSwapModalOpen = false;
            await invalidateAll();
        } catch (err: any) {
            showToast(err.message || 'Gagal menukar jadwal', 'error');
        } finally {
            isSubmittingSwap = false;
        }
    }

    // --- Modal 3: Bulk Assign Modal (Standard) ---
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-muted-foreground">Kelola kalender roster kerja, rotasi shift otomatis, hari libur bergilir, dan master shift operasional.</p>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            ...
            {:else if activeTab === 'roster'}
                <button 
                    ...
                    class="inline-flex items-center gap-2 px-3.5 py-2 border border-input bg-background hover:bg-muted text-foreground rounded-lg text-sm font-medium transition-colors"
                    title="Tukar jadwal shift / libur antara 2 karyawan"
                >
                    <ArrowRightLeft class="w-4 h-4 text-blue-500" /> Tukar Shift / Libur
                </button>
                <button 
                    ...
                    title="Buat roster otomatis sebulan dengan rotasi rolling"
                >
                    <Sparkles class="w-4 h-4" /> Auto-Generate Roster
                </button>
                <button 
                    onclick={() => openRosterModal(selectedDate)}
                    class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 text-sm font-medium transition-colors"
                >
                    <Plus class="w-4 h-4" /> Tetapkan Manual
                </button>
            {/if}
        </div>
    </div>
    ...
    <!-- Tab 1: Kalender & Penjadwalan Roster -->
    {#if activeTab === 'roster'}
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                ...
                <!-- Right (5 Cols): Selected Date Employee Shift Panel with 1-Click Select All -->
                <div class="lg:col-span-5 bg-card border border-border rounded-xl p-4 space-y-4">
                    ...
                    <!-- 1-Click Master Checkbox Header & Bulk Action Bar -->
                    <div class="p-2.5 bg-muted/30 rounded-lg border border-border/80 space-y-2">
                        <div class="flex items-center justify-between">
                            <label class="inline-flex items-center gap-2 cursor-pointer text-xs font-semibold text-foreground">
                                <input 
                                    type="checkbox" 
                                    checked={isAllDateEmployeesChecked}
                                    onchange={toggleAllDateEmployees}
                                    class="rounded border-border text-primary focus:ring-0"
                                >
                                <span>{isAllDateEmployeesChecked ? 'Batalkan Semua' : 'Centang Semua'} ({selectedDateEmployees.length})</span>
                            </label>

                            {#if selectedDateCheckedEmpIds.length > 0}
                                <span class="text-[11px] font-bold text-primary">
                                    {selectedDateCheckedEmpIds.length} Dipilih
                                </span>
                            {/if}
                        </div>
                        ...
                        {#if selectedDateCheckedEmpIds.length > 0}
                            <div transition:slide class="pt-2 border-t border-border flex items-center gap-2 flex-wrap">
                                <select
                                    bind:value={bulkDateShiftId}
                                    class="h-8 px-2 bg-background border border-border rounded-md text-xs font-medium outline-none focus:border-primary flex-1 min-w-[120px]"
                                >
                                    <option value="">Pilih Shift Baru...</option>
                                    {#each shifts as s}
                                        <option value={s.id}>{s.name} ({s.work_start_time?.substring(0, 5)})</option>
                                    {/each}
                                    <option value="remove">Jadikan Reguler / Libur</option>
                                </select>
                                <button
                                    onclick={() => handleApplyBulkDateShift(bulkDateShiftId)}
                                    disabled={!bulkDateShiftId || isApplyingBulkDate}
                                    class="px-3 py-1.5 bg-primary text-primary-foreground rounded-md text-xs font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors"
                                >
                                    {isApplyingBulkDate ? 'Memproses...' : 'Terapkan'}
                                </button>
                            </div>
                        {/if}
                    </div>
                    ...
                    <div class="max-h-[440px] overflow-y-auto space-y-1.5 pr-1 divide-y divide-border/40">
                        ...
                        {:else}
                            {#each selectedDateEmployees as item}
                                <div class="pt-2 flex items-center justify-between gap-2.5">
                                    <div class="flex items-center gap-2.5 min-w-0 flex-1">
                                        <input 
                                            type="checkbox" 
                                            checked={selectedDateCheckedEmpIds.includes(item.employee.id)}
                                            onchange={() => toggleSingleDateEmployee(item.employee.id)}
                                            class="rounded border-border text-primary focus:ring-0"
                                        >
                                    ...
                                    <div class="flex items-center gap-1.5 flex-shrink-0">
                                        <select
                                            ...
                                            class="h-8 px-2 bg-background border border-border rounded-lg text-xs font-medium outline-none focus:border-primary cursor-pointer max-w-[130px]"
...
{#if isAutoGenerateModalOpen}
    <div class="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl max-w-xl w-full p-5 space-y-4 shadow-xl relative max-h-[92vh] flex flex-col">
            <div class="flex items-center justify-between pb-3 border-b border-border flex-shrink-0">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-foreground">Auto-Generate Roster Shift Cerdas</h3>
                        <p class="text-[11px] text-muted-foreground">Susun jadwal kerja sebulan otomatis dengan rotasi rolling & anti-bentrok</p>
                    </div>
                </div>
                <button onclick={() => isAutoGenerateModalOpen = false} class="w-7 h-7 rounded-full bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground">
                    <X class="w-4 h-4" />
                </button>
            </div>
            ...
            <div class="space-y-4 text-xs overflow-y-auto pr-1">
                <div class="space-y-1.5">
                    <label class="block font-semibold text-foreground">1. Rentang Tanggal Roster</label>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <span class="text-[10px] text-muted-foreground">Dari Tanggal:</span>
                            <input type="date" bind:value={autoGenForm.start_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary mt-0.5" />
                        </div>
                        <div>
                            <span class="text-[10px] text-muted-foreground">Sampai Tanggal:</span>
                            <input type="date" bind:value={autoGenForm.end_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary mt-0.5" />
                        </div>
                    </div>
                </div>

                <div class="space-y-1.5 pt-2 border-t border-border">
                    <label class="block font-semibold text-foreground">2. Pilih Master Shift yang Digunakan</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {#each shifts as s}
                            <label class="flex items-center gap-2 p-2 rounded-lg border border-border/80 bg-muted/20 hover:bg-muted/40 cursor-pointer">
                                <input 
                                    type="checkbox" 
                                    checked={autoGenForm.shift_ids.includes(s.id)}
                                    onchange={() => toggleAutoGenShift(s.id)}
                                    class="rounded border-border text-primary focus:ring-0"
                                >
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-[10px] text-muted-foreground">{s.work_start_time?.substring(0, 5)} - {s.work_end_time?.substring(0, 5)} WIB</p>
                                </div>
                            </label>
                        {/each}
                    </div>
                ...
                <div class="space-y-1.5 pt-2 border-t border-border">
                    <label class="block font-semibold text-foreground">3. Pola Rotasi Kerja (Shift Pattern)</label>
                    <div class="grid grid-cols-1 gap-2">
                        <label class="p-2.5 rounded-lg border cursor-pointer flex items-start gap-2.5 {autoGenForm.pattern === 'weekly_rotation' ? 'border-primary bg-primary/5' : 'border-border bg-muted/10'}">
                            <input type="radio" value="weekly_rotation" bind:group={autoGenForm.pattern} class="mt-0.5 text-primary focus:ring-0">
                            <div>
                                <p class="font-semibold text-foreground">Rotasi Bergilir Mingguan (Rolling Shifts)</p>
                                <p class="text-[10px] text-muted-foreground">Karyawan berganti shift setiap hari Senin (contoh: Minggu ini Shift Pagi, Minggu depan otomatis Shift Siang, dst).</p>
                            </div>
                        </label>

                        <label class="p-2.5 rounded-lg border cursor-pointer flex items-start gap-2.5 {autoGenForm.pattern === 'balanced_daily' ? 'border-primary bg-primary/5' : 'border-border bg-muted/10'}">
                            <input type="radio" value="balanced_daily" bind:group={autoGenForm.pattern} class="mt-0.5 text-primary focus:ring-0">
                            <div>
                                <p class="font-semibold text-foreground">Distribusi Seimbang (Balanced Equal)</p>
                                <p class="text-[10px] text-muted-foreground">Membagi jumlah karyawan secara merata ke shift yang aktif setiap hari.</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div class="space-y-1.5 pt-2 border-t border-border">
                    <label class="block font-semibold text-foreground">4. Aturan Hari Libur (Day-Off)</label>
                    <div class="flex items-center gap-3 flex-wrap">
                        <label class="inline-flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                            <input type="checkbox" checked={autoGenForm.off_days.includes(0)} onchange={() => toggleOffDay(0)} class="rounded border-border text-primary focus:ring-0">
                            <span>Minggu Libur</span>
                        </label>
                        <label class="inline-flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                            <input type="checkbox" checked={autoGenForm.off_days.includes(6)} onchange={() => toggleOffDay(6)} class="rounded border-border text-primary focus:ring-0">
                            <span>Sabtu Libur</span>
                        </label>
                        <label class="inline-flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                            <input type="checkbox" checked={autoGenForm.off_days.includes(5)} onchange={() => toggleOffDay(5)} class="rounded border-border text-primary focus:ring-0">
                            <span>Jumat Libur</span>
                        </label>
                    </div>

                    <div class="mt-2 p-2.5 bg-muted/30 rounded-lg border border-border flex items-center justify-between gap-3">
                        <div class="space-y-0.5">
                            <p class="text-xs font-semibold text-foreground">Rotasi Hari Libur Bergilir (Rotating Day-Off)</p>
                            <p class="text-[10px] text-muted-foreground">Jatah libur karyawan bergantian di hari kerja/akhir pekan agar operasional tetap jalan.</p>
                        </div>
                        <input type="checkbox" bind:checked={autoGenForm.rotating_off} class="rounded border-border text-primary focus:ring-0">
                    </div>
                </div>

                <div class="space-y-2 pt-2 border-t border-border">
                    <div class="flex items-center justify-between">
                        <label class="font-semibold text-foreground">5. Karyawan ({autoGenForm.employee_ids.length} dari {employees.length} Terpilih)</label>
                        <div class="flex items-center gap-1.5">
                            <button
                                type="button"
                                onclick={() => autoGenForm.employee_ids = employees.map(e => e.id)}
                                class="px-2 py-0.5 rounded bg-muted hover:bg-muted/80 text-[10px] font-medium"
                            >
                                Pilih Semua
                            </button>
                            <button
                                type="button"
                                onclick={() => autoGenForm.employee_ids = []}
                                class="px-2 py-0.5 rounded bg-muted hover:bg-muted/80 text-[10px] font-medium"
                            >
                                Batal Pilih
                            </button>
                    ...
                    <div class="relative">
                        ...
                        <input
                            type="text"
                            bind:value={autoGenSearchQuery}
                            placeholder="Cari nama karyawan..."
                            class="w-full h-8 pl-8 pr-3 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary"
                        />
                    </div>

                    <div class="max-h-36 overflow-y-auto border border-border rounded-lg p-2 space-y-1 bg-muted/10 divide-y divide-border/40">
                        {#each autoGenFilteredEmployees as emp}
                            <label class="flex items-center gap-2 p-1.5 hover:bg-muted/60 rounded cursor-pointer">
                                <input 
                                    type="checkbox" 
                                    checked={autoGenForm.employee_ids.includes(emp.id)}
                                    onchange={() => toggleAutoGenEmployee(emp.id)}
                                    class="rounded border-border text-primary focus:ring-0"
                                >
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-foreground truncate">{emp.user?.name || '-'}</p>
                                    <p class="text-[10px] text-muted-foreground truncate">{emp.department || '-'}</p>
                                </div>
                            </label>
                        {/each}
                    </div>
            ...
            <div class="flex items-center justify-end gap-2 pt-3 border-t border-border flex-shrink-0">
                <button onclick={() => isAutoGenerateModalOpen = false} class="px-3.5 py-2 rounded-lg border border-border text-xs font-medium hover:bg-muted">
                    Batal
                </button>
                <button 
                    onclick={handleExecuteAutoGenerate} 
                    disabled={isSubmittingAutoGenerate} 
                    class="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-medium shadow-sm transition-colors"
                >
                    <Sparkles class="w-3.5 h-3.5" />
                    <span>{isSubmittingAutoGenerate ? 'Menyusun Roster...' : 'Generate Jadwal Roster Sekarang'}</span>
                </button>
            </div>
        </div>
    </div>
...
<!-- MODAL 2: Tukar Jadwal Antar Karyawan (Swap Schedule with Search) -->
{#if isSwapModalOpen}
    <div class="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl max-w-xl w-full p-5 space-y-4 shadow-xl relative max-h-[92vh] flex flex-col">
            <div class="flex items-center justify-between pb-3 border-b border-border flex-shrink-0">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-foreground">Tukar Jadwal Antar Karyawan</h3>
                        <p class="text-[11px] text-muted-foreground">Tukar shift atau hari libur antara dua orang karyawan</p>
                    </div>
                </div>
                <button onclick={() => isSwapModalOpen = false} class="w-7 h-7 rounded-full bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground">
                    <X class="w-4 h-4" />
                </button>
            </div>
            ...
            <div class="space-y-4 text-xs overflow-y-auto pr-1">
                ...
                <!-- Date Range of Swap -->
                <div class="space-y-1">
                    <label class="block font-semibold text-foreground">Rentang Tanggal Pertukaran</label>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <span class="text-[10px] text-muted-foreground">Dari Tanggal:</span>
                            <input type="date" bind:value={swapForm.start_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary mt-0.5" />
                        </div>
                        <div>
                            <span class="text-[10px] text-muted-foreground">Sampai Tanggal:</span>
                            <input type="date" bind:value={swapForm.end_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary mt-0.5" />
                        </div>
                    </div>
                </div>

                <!-- Two Column Employee Selector with Search -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-border">
                    
                    <!-- Karyawan 1 Search & Select -->
                    <div class="space-y-2 p-3 bg-muted/20 rounded-xl border border-border">
                        <div class="flex items-center justify-between">
                            ...
                            {#if selectedEmp1Obj}
                                <span class="text-[10px] font-semibold text-primary truncate max-w-[120px]">
                                    {selectedEmp1Obj.user?.name}
                                </span>
                            {/if}
                        </div>

                        <div class="relative">
                            <Search class="w-3 h-3 absolute left-2.5 top-2.5 text-muted-foreground" />
                            <input
                                type="text"
                                bind:value={searchEmp1}
                                placeholder="Cari nama karyawan 1..."
                                class="w-full h-8 pl-7 pr-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary"
                            />
                        </div>

                        <div class="max-h-36 overflow-y-auto border border-border rounded-lg p-1.5 space-y-1 bg-background">
                            {#if filteredSwapEmployees1.length === 0}
                                <p class="text-center py-2 text-[10px] text-muted-foreground">Tidak ditemukan</p>
                            {:else}
                                {#each filteredSwapEmployees1 as emp}
                                    <button
                                        type="button"
                                        onclick={() => swapForm.employee_id_1 = emp.id}
                                        class="w-full text-left p-1.5 rounded flex items-center justify-between transition-colors {Number(swapForm.employee_id_1) === emp.id ? 'bg-primary text-primary-foreground font-semibold' : 'hover:bg-muted text-foreground'}"
                                    >
                                        <div class="min-w-0 flex-1">
                                            <p class="text-xs truncate">{emp.user?.name || '-'}</p>
                                            <p class="text-[9px] opacity-75 truncate">{emp.department || '-'}</p>
                                        </div>
                                        {#if Number(swapForm.employee_id_1) === emp.id}
                    ...
                    <div class="space-y-2 p-3 bg-muted/20 rounded-xl border border-border">
                        <div class="flex items-center justify-between">
                            ...
                            {#if selectedEmp2Obj}
                                <span class="text-[10px] font-semibold text-emerald-600 truncate max-w-[120px]">
                                    {selectedEmp2Obj.user?.name}
                                </span>
                            {/if}
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                type="text"
                                bind:value={searchEmp2}
                                placeholder="Cari nama karyawan 2..."
                                class="w-full h-8 pl-7 pr-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary"
                            />
                        </div>

                        <div class="max-h-36 overflow-y-auto border border-border rounded-lg p-1.5 space-y-1 bg-background">
                            {#if filteredSwapEmployees2.length === 0}
                                <p class="text-center py-2 text-[10px] text-muted-foreground">Tidak ditemukan</p>
                            {:else}
                                {#each filteredSwapEmployees2 as emp}
                                    <button
                                        type="button"
                                        onclick={() => swapForm.employee_id_2 = emp.id}
                                        class="w-full text-left p-1.5 rounded flex items-center justify-between transition-colors {Number(swapForm.employee_id_2) === emp.id ? 'bg-emerald-600 text-white font-semibold' : 'hover:bg-muted text-foreground'}"
                                    >
                                        <div class="min-w-0 flex-1">
                                            <p class="text-xs truncate">{emp.user?.name || '-'}</p>
                                            <p class="text-[9px] opacity-75 truncate">{emp.department || '-'}</p>
                                        </div>
                                        {#if Number(swapForm.employee_id_2) === emp.id}
                                            <Check class="w-3.5 h-3.5 flex-shrink-0" />
                                        {/if}
                                    </button>
                                {/each}
                ...
                <!-- Swap Preview Info Banner -->
                {#if selectedEmp1Obj && selectedEmp2Obj && Number(swapForm.employee_id_1) !== Number(swapForm.employee_id_2)}
                    <div class="p-3 bg-blue-50/70 dark:bg-blue-950/40 rounded-xl border border-blue-200 dark:border-blue-800/60 flex items-center gap-3">
                        <ArrowRightLeft class="w-4 h-4 text-blue-500 flex-shrink-0" />
                        <p class="text-xs text-blue-700 dark:text-blue-300">
                            Jadwal shift dan hari libur antara <b>{selectedEmp1Obj.user?.name}</b> dan <b>{selectedEmp2Obj.user?.name}</b> akan saling bertukar pada rentang <b>{swapForm.start_date}</b> s/d <b>{swapForm.end_date}</b>.
                        </p>
                    </div>
                {/if}
            ...
            <div class="flex items-center justify-end gap-2 pt-3 border-t border-border flex-shrink-0">
                <button onclick={() => isSwapModalOpen = false} class="px-3.5 py-2 rounded-lg border border-border text-xs font-medium hover:bg-muted">
                    Batal
                </button>
                <button onclick={handleExecuteSwap} disabled={isSubmittingSwap} class="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-xs font-medium hover:bg-primary/90">
                    {isSubmittingSwap ? 'Menukar...' : 'Tukar Jadwal Sekarang'}
                </button>
            </div>
        </div>
    </div>
...
<!-- MODAL 3: Tetapkan Shift Manual / Standar -->
...
<!-- MODAL 4: Tambah/Edit Master Shift -->
{#if isShiftModalOpen}
    <div class="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
...
console.log('Successfully updated +page.svelte with search filters for Swap Modal!');
