...
const newSvelteContent = `<script lang="ts">
    ...
    let policies = $state<any>(data.policies || {});

    // Filters for balance tables
    let search = $state('');
        ...
        try {
            const res = await fetch(\`\${PUBLIC_API_URL}/hris/request-policies\`, {
                ...
                body: JSON.stringify({
                    ...
                    max_days_per_year: p.max_days_per_year !== undefined ? p.max_days_per_year : 14,
    ...
    <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in-95">
            <div class="flex items-center justify-between border-b border-border pb-3">
                ...
                <button onclick={() => isTypeModalOpen = false} class="text-muted-foreground hover:text-foreground text-xs font-bold">✕</button>
            </div>
    ...
    <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-4 animate-in fade-in zoom-in-95">
            <div class="flex items-center justify-between border-b border-border pb-3">
                ...
                <button onclick={() => isEditBalanceModalOpen = false} class="text-muted-foreground hover:text-foreground text-xs font-bold">✕</button>
            </div>
...
console.log('Successfully written redesigned pengaturan-cuti/+page.svelte with 4 independent tabs and clean Lucide icons!');
