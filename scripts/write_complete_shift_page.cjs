...
const pageContent = `<script lang="ts">
    ...
    // Multi-Select checkboxes in Selected Date Panel
    ...
    // 1-Click Master Checkbox Toggle for Selected Date Panel
    ...
    // Apply Bulk Shift for Checked Employees on Selected Date
    async function handleApplyBulkDateShift(targetShiftId: string | number) {
        ...
        try {
            if (!targetShiftId || targetShiftId === 'remove') {
                // Remove shift for checked employees on selected date
                const dayAssignments = assignmentsByDate[selectedDate] || [];
    ...
    // Quick Change Shift directly for a single employee
    ...
    let autoGenForm = $state({
        start_date: todayStr,
        end_date: new Date(todayObj.getFullYear(), todayObj.getMonth() + 1, 0).toISOString().split('T')[0], // end of month
        employee_ids: [] as number[],
        shift_ids: [] as number[],
        pattern: 'weekly_rotation', // weekly_rotation, balanced_daily, fixed_shift
        off_days: [0], // 0 = Sunday
        rotating_off: false,
    ...
    // --- Modal 2: Swap / Tukar Jadwal Antar Karyawan ---
...
<div class="space-y-6">
    ...
    <!-- Tab 1: Kalender & Penjadwalan Roster (Calendar Split View) -->
    {#if activeTab === 'roster'}
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                ...
                <div class="lg:col-span-5 bg-card border border-border rounded-xl p-4 space-y-4">
                    ...
                    <!-- Search & Filter Controls (NO 'NIK' in placeholder) -->
                    <div class="space-y-2">
                        <div class="relative">
    ...
    <div class="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl max-w-xl w-full p-5 space-y-4 shadow-xl relative max-h-[92vh] flex flex-col">
            ...
            <div class="space-y-4 text-xs overflow-y-auto pr-1">
                ...
                <!-- 5. Employee List (No NIK in search) -->
...
<!-- MODAL 2: Tukar Jadwal Antar Karyawan (Swap Schedule) -->
{#if isSwapModalOpen}
    <div class="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-card border border-border rounded-2xl max-w-md w-full p-5 space-y-4 shadow-xl relative">
            ...
            <div class="space-y-3.5 text-xs">
                <div>
                    <label class="block font-medium text-foreground mb-1">Karyawan Pertama</label>
                    <select bind:value={swapForm.employee_id_1} class="w-full h-9 px-3 bg-background border border-border rounded-lg text-sm outline-none focus:border-primary">
                        {#each employees as e}
                            <option value={e.id}>{e.user?.name || '-'} ({e.department || '-'})</option>
                        {/each}
                    </select>
                </div>

                <div class="flex justify-center my-1">
                    <div class="w-7 h-7 rounded-full bg-muted flex items-center justify-center text-muted-foreground">
                        <ArrowRightLeft class="w-3.5 h-3.5" />
                    </div>
                </div>

                <div>
                    <label class="block font-medium text-foreground mb-1">Karyawan Kedua</label>
                    <select bind:value={swapForm.employee_id_2} class="w-full h-9 px-3 bg-background border border-border rounded-lg text-sm outline-none focus:border-primary">
                        {#each employees as e}
                            <option value={e.id}>{e.user?.name || '-'} ({e.department || '-'})</option>
                        {/each}
                    </select>
                ...
                <div class="grid grid-cols-2 gap-3 pt-1 border-t border-border">
                    <div>
                        <label class="block font-medium text-foreground mb-1">Dari Tanggal</label>
                        <input type="date" bind:value={swapForm.start_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary" />
                    </div>
                    <div>
                        <label class="block font-medium text-foreground mb-1">Sampai Tanggal</label>
                        <input type="date" bind:value={swapForm.end_date} class="w-full h-9 px-2.5 bg-background border border-border rounded-lg text-xs outline-none focus:border-primary" />
                    </div>
                </div>
            </div>
...
console.log('Successfully written complete Svelte shift page with Auto-Generate Roster, Swap Shifts, and 1-Click Multi-Select!');
