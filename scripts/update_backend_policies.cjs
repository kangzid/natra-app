...
// 1. Update HrisLeaveSettingController.php
const ctrlPath = path.join(backendDir, 'app/Http/Controllers/Api/HrisLeaveSettingController.php');
...
console.log('Updated HrisLeaveSettingController.php with complete separated modules!');

// 2. Update routes/api.php with policy routes
const apiRoutesPath = path.join(backendDir, 'routes/api.php');
let apiRoutes = fs.readFileSync(apiRoutesPath, 'utf8');

if (!apiRoutes.includes('hris/request-policies')) {
    const routeInsert = `        Route::get('/hris/request-policies', [\\App\\Http\\Controllers\\Api\\HrisLeaveSettingController::class, 'getPolicies']);
    ...
    apiRoutes = apiRoutes.replace("Route::get('/leave-types'", routeInsert + "        Route::get('/leave-types'");
    fs.writeFileSync(apiRoutesPath, apiRoutes, 'utf8');
    console.log('Updated routes/api.php with request-policies routes!');
}
